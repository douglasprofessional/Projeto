...
if ( ! function_exists( 'catchresponsive_setup' ) ) :
	...
	function catchresponsive_setup() {
		...
		add_theme_support( 'custom-background', apply_filters( 'catchresponsive_custom_background_args', array(
			...
			'default-image'	=> trailingslashit( esc_url ( get_template_directory_uri() ) ) . 'images/' . $default_bg_image,
		) ) );
...
function catchresponsive_scripts() {
	...
	wp_enqueue_style( 'catchresponsive-style', get_stylesheet_uri(), null, date( 'Ymd-Gis', filemtime( get_template_directory() . '/style.css' ) ) );

	wp_enqueue_script( 'catchresponsive-navigation', trailingslashit( esc_url ( get_template_directory_uri() ) ) . 'js/navigation.min.js', array(), '20120206', true );

	// Load the html5 shiv.
	wp_enqueue_script( 'catchresponsive-html5', trailingslashit( esc_url ( get_template_directory_uri() ) ) . 'js/html5.min.js', array(), '3.7.3' );
	wp_script_add_data( 'catchresponsive-html5', 'conditional', 'lt IE 9' );

	wp_enqueue_script( 'catchresponsive-skip-link-focus-fix', trailingslashit( esc_url ( get_template_directory_uri() ) ) . 'js/skip-link-focus-fix.min.js', array(), '20130115', true );
	...
	if ( $options['color_scheme'] != 'light' ) {
		wp_enqueue_style( 'catchresponsive-dark', trailingslashit( esc_url ( get_template_directory_uri() ) ) . 'css/colors/dark.css', array(), null );
	}
	...
	if ( ! $options['disable_scrollup'] ) {
		wp_enqueue_script( 'catchresponsive-scrollup', trailingslashit( esc_url ( get_template_directory_uri() ) ) . 'js/catchresponsive-scrollup.min.js', array( 'jquery' ), '20072014', true  );
	}

	/**
	 * Enqueue custom script for catchresponsive.
	 */
	wp_enqueue_script( 'catchresponsive-custom-scripts', trailingslashit( esc_url ( get_template_directory_uri() ) ) . 'js/catchresponsive-custom-scripts.min.js', array( 'jquery' ), null );
}
...
if ( ! function_exists( 'catchresponsive_page_post_meta' ) ) :
	...
	function catchresponsive_page_post_meta() {
	    ...
	    $catchresponsive_page_post_meta .= '<span class="post-author">' . __( 'By', 'catch-responsive' ) . ' <span class="author vcard"><a class="url fn n" href="' . esc_url( $catchresponsive_author_url ) . '" title="View all posts by ' . esc_attr( get_the_author() ) . '" rel="author">' .get_the_author() . '</a></span>';
